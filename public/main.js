// NEW query string will be without date range.  ie:
// https://api.nasa.gov/neo/rest/v1/feed?[DATE VARIABLE HERE?]&api_key=
// FRONT END JS:
...
$(document).ready(function(){
  ...
  $('#asteroid-form').on('submit', function(event){
    ...
    $.post('/nasa_api', { date: asteroidDate }).then(function(data){
      ...
      for (date in neo) { // loops through all first objects in near_earth_objects
      ...
      for (let i=0; i < dangerousAsteroids.length; i++) {
        //we have created our array of just the asteroids that returned "true" as dangerous, we can now use jQuery to .append the results into our html using a template string.
        $("#asteroids").append(`
          <section>
            <h3>Asteroid Name:</h3>
